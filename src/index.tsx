    ...
    Alert.alert("Not found Twitter App","Please make a choice",[
      {
        ...
        "onPress": async () => {
          let isUrl = await Linking.canOpenURL(`https://twitter.com/intent/tweet?text=${this.msg}&url=${this.url || null}&hashtags=${this.hashtags || null}`)
          if(isUrl){
            Linking.openURL(`https://twitter.com/intent/tweet?text=${this.msg}&url=${this.url || null}&hashtags=${this.hashtags || null}`)
          }
        }
      },{
        "text": "Download on App Store",
        "onPress": async () => {
          let isUrl = await Linking.canOpenURL("market://details?id=com.twitter.android")
          if(isUrl){
